...
let string_of_info info =
  ...
  | InfoFun (i,n,t,tp) -> "Fonction "^n^(string_of_int i)^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                          " -> "^(string_of_type t)
  | InfoEnumVal (n,t,dep,base) -> "Valeur Enumeration "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^base^"]"
  | InfoEnum (n) -> "Enumeration "^n
...
let modifier_type_fonction_info t tp i =
  match !i with
  |InfoFun(indice,n,_,_) -> i:= InfoFun(indice,n,t,tp)
...
let modifier_type_enum_info t i = 
  match !i with
  | InfoEnumVal(n,_,dep,base) -> i:= InfoEnumVal(n,t,dep,base)
  | _ -> failwith "Appel modifier_type_enum_info pas sur un InfoEnumVal"

let modifier_adresse_enum_info d b i =
  match !i with
  |InfoEnumVal (n,t,_,_) -> i:= InfoEnumVal (n,t,d,b)
  | _ -> failwith "Appel modifier_adresse_enum_info pas sur un InfoVar"

let rec chercher_fonction tds nom =
  match tds with
  | Nulle -> []
  | Courante (m,c) ->
      match find_all c nom with
      | [] -> chercher_fonction m nom
      | ia -> ia@(chercher_fonction m nom)