...
struct
  ...
  let analyser_fonction (Ast.AstPlacement.Fonction(info,lp,li,_)) =
    ...
    | InfoFun(_,n,_,_) -> [(n,(List.flatten (List.map analyser_param lp))@(List.flatten (List.map (analyser_instruction) li)))]
    | _ -> failwith "Internal error"

  (* Renvoie la suite des adresses où sont stockés les valeurs d'une énumération *) 
  let analyser_enumeration (Ast.AstType.Enumeration(info,li)) =
    let rec aux l = 
      begin
        match l with
        | [] -> []
        | t::q -> 
            begin
              ...
              | (InfoEnumVal(n,_,d,r)) -> [(n,(d,r))]@(aux q)
              | _ -> failwith "Internal error"
            end
      end
    in
    ...
    | InfoEnum(n) -> [(n,(aux li))]
    | _ -> failwith "Internal error"
  (* Renvoie la suite des adresses des variables déclarées dans les énumérations, les fonctions et dans le programme principal *)
  let analyser (Ast.AstPlacement.Programme (enumerations,fonctions, prog)) =
    let l = ("main", List.flatten (List.map (analyser_instruction) prog))::(List.flatten (List.map (analyser_fonction) fonctions)) in 
    l@(List.flatten (List.map (analyser_enumeration) enumerations))
