...
type info =
  ...
  (* On ajoute un identifiant à la fonction pour gérer le surcharge des fonctions *)
  | InfoFun of int * string * typ * typ list 
  (* InfoEnumVal est une info où on stocke les informations nécessaire
  sur une valeur de type énuéré, son nom, le type énuméré correspondant, son déplacement
  par rapport à un registre*)
  | InfoEnumVal of string * typ * int * string
    (* InfoEnum est une info où on stocke le nom d'une énumération*)
  | InfoEnum of string 
(* Table des symboles *)
type tds 
...
(* Fonction pour modifier l'adresse d'une valeur énumérée (Où elle est stockée)*)
val modifier_adresse_enum_info : int -> string -> info_ast -> unit


(* Fonction pour modifier le type enum d'une valeur énumérée *)
val modifier_type_enum_info : typ -> info_ast -> unit

(* Fonction pour rechercher les fonctions qui ont le même nom (nom rentrée)
en paramètre, elle renvoie une liste des infoast correspondants*)
 val chercher_fonction : tds -> string -> (info_ast list)